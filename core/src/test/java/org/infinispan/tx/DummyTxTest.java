...
public class DummyTxTest extends SingleCacheManagerTest {
   ...
   protected EmbeddedCacheManager createCacheManager() throws Exception {
      ...
      cb.clustering().stateTransfer().fetchInMemoryState(false)
            .invocationBatching().enable()
            .versioning().enable().scheme(VersioningScheme.SIMPLE)
            .locking().lockAcquisitionTimeout(1000).writeSkewCheck(true).isolationLevel(IsolationLevel.REPEATABLE_READ);
   ...
   public void testConcurrentRemove() throws Exception {
      ...
      for (int i = 0; i < numThreads; i++) {
         threads[i] = new Thread("Remover-" + i) {
            public void run() {
               try {
                  latch.await();
