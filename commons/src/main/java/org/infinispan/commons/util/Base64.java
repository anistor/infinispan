...
/**
 * Encodes and decodes to and from Base64 notation. <p> Change Log: </p>
 ...
 * was a single byte.</li> <li>v2.0 - I got rid of methods that used booleans
 * to set options. Now everything is more consolidated and cleaner. The code
 * now detects when data that's being decoded is gzip-compressed and will
 * decompress it automatically. Generally things are cleaner. You'll probably
 * have to change some method calls that you were making to support the new
 * options format (<tt>int</tt>s that you "OR" together).</li> <li>v1.5.1 -
 * Fixed bug when decompressing and decoding to a byte[] using <tt>decode(
 ...
 * file).</li> <li>v1.5 - Output stream pases on flush() command but doesn't do
 * anything itself. This helps when using GZIP streams. Added the ability to
 * GZip-compress objects before encoding them.</li> <li>v1.4 - Added helper
 * methods to read/write files.</li> <li>v1.3.6 - Fixed OutputStream.flush() so
 * that 'position' is reset.</li> <li>v1.3.5 - Added flag to turn on and off
 ...
 * I/O streams which were totally messed up.</li> </ul> <p> I am placing
